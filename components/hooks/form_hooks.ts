...
const RegistrationFormSchema = z.object({
    username: z.string()
        .regex(/^[a-zA-Z0-9_]*$/, {message: "使用者名稱只能包含字母、數字和底線"}),
    password: z.string()
...
export function useUserLoginForm() {
    ...
    const [isPasswordVisible, togglePasswordVisibility] = useToggleVisibility(false)
...
export function useUserRegistrationForm() {
    ...
    const [registrationErrorMessage, setRegistrationError] = useState<string | null>(null);
    const [isValidationPasswordVisible, toggleValidationPasswordVisibility] = useToggleVisibility(false)
