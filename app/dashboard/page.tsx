    ...
    >
        ...
        <div className="flex flex-col justify-center items-start gap-[15px] self-stretch p-5">
            ...
            <div className="flex items-start content-start gap-2.5 self-stretch flex-wrap">
                {tags.map((tag, index) => (
                    <div key={index} className="text-sm not-italic font-bold leading-[normal] flex justify-center items-center gap-[5px] px-2.5 py-1"
                        style={{ background: `linear-gradient(0deg, rgba(255, 255, 255, 0.95) 0%, rgba(255, 255, 255, 0.95) 100%), ${theme}` }}>
                        <span>#</span>
                        <span>{tag}</span>
                    </div>
                ))}
            ...
            <div className="flex flex-col justify-center items-end gap-2.5 self-stretch">
                <button className="text-white text-xs not-italic font-bold leading-4 flex flex-row items-center gap-1 px-2.5 py-[5px] rounded-[5px]"
                    style={{ background: theme }}>
                    <span><Link href={link.path}>{link.label}</Link></span>
                    <span><HiArrowRight style={{ height: 24, width: 24 }} /></span>
                </button>
            </div>
        </div>
    </div>
