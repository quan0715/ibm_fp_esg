...
import { ExternalInspection, ExternalInspectionReport, } from "../_fake_data_type"
import { cn } from "@nextui-org/react"

import { Button } from "@/components/ui/button"
import DataTable, { DataTableRefType, Filter } from "@/app/dashboard/_components/DataTable";

export default function Component({ data, className }: { data: ExternalInspectionReport[], className?: string }) {
    const factories = React.useMemo(() => ["烯烴部", "保養中心", "碼槽處", "公用部", "公務部", "煉油部"], [])

    const [factoryFilterArr, setFactoryFilterArr] = React.useState<string[]>([])

    const dataColumns = React.useMemo<(keyof ExternalInspection)[]>(() => (Object.keys(data[0]["外稽檢核"][0]) as (keyof ExternalInspection)[]), [])
