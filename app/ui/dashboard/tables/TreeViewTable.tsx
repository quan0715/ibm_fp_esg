...
function updateExpandStatus(
    ...
    expandedStatus: boolean): TreeViewTableDataType[]{
    const childrenIndexList = item.childrenIndex

    var childData = treeData.filter(
        (treeDataItem) => childrenIndexList.includes(treeDataItem.index)
    )
    ...
    if (childrenIndexList.length === 0){
        ...
        for(let i = 0; i < childrenIndexList.length; i++) {
               arr = arr.concat(updateExpandStatus(childData[i], treeData, expandedStatus))
        }
        return res.concat(arr)
    }
}

export function OperationDataTreeViewTable({data} : {data: AssetOperationDataSchemaModel[]}) {

    const [selectedKey, setSelectedKey] = useState([""]);
    const [selectedData, setSelectedData] = useState<AssetOperationDataSchemaModel | null>(null);
    // const [hiddenNodes, setHiddenNodes] = useState<number[]>([]);
    const [treeData, setTreeData] = useState<TreeViewTableDataType[]>(data.map((item) => {
        return {
            ...item,
            isHidden: false,
            isExpanded: true
    ...
    {name: "Asset Name", uid:"assetName", sortable: true, renderCell: (item, value) => {
            ...
            <div className={"flex flex-row items-center"}>
                ...
                <button onClick={
                        (e) => {
                            ...
                            const itemIndex = treeData.findIndex((treeDataItem) => treeDataItem.index === item.index)
                            const childrenIndex = item.childrenIndex
                            const expandedStatus = !item.isExpanded
                            const updatedTreeData = updateExpandStatus(item, treeData, expandedStatus)
                            const updatedIndexList = updatedTreeData.map((item) => item.index)

                            console.log('updatedIndexList', updatedIndexList)
                            console.log('updated', updatedTreeData)
                            setTreeData(treeData.map((treeDataItem)=> {
                               if(treeDataItem.index === item.index){
                                   ...
                                   let updatedItem = updatedTreeData.find((item) => item.index === treeDataItem.index)
                                   if(updatedItem) {
                                      return {
                                          ...updatedItem
                                      }
    ...
    {name: "發電量", uid:"powerGeneration", sortable: true, renderCell: (item, value) => {
    ...
    {name: "風速", uid:"windSpeed", sortable: true, renderCell: (item, value) => {
    ...
    {name: "溫度", uid:"temperature", sortable: true, renderCell: (item, value) => {
        const temperature = value as number
        return temperature  !== -1
            ? (<div>{`${value} ºC`}</div>)
            : (<div> </div>)
    }},
    {name: "發電狀態", uid:"powerGenerationStatus", sortable: true, renderCell: (item, value) => {
    ...
    const renderCell = ({item, columnKey} : {item: TreeViewTableDataType, columnKey: React.Key }) => {
    ...
    return (
        <DashboardCard title={"Asset Performance List"} className={"flex-grow"}>
            <Table
                radius={"none"} border={0} color={"primary"} shadow={"none"}
                removeWrapper={false} selectionMode={"single"} selectionBehavior={"replace"}
                ...
                <TableBody emptyContent={"no items to display"} items={treeData}>{
                    (data) => (<TableRow key={data.index} onClick={() => {
