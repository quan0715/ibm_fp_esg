import { MongoLocationDataModel } from "@/data/models/mongo/LocationModel";
import {
  LocationType,
  getLocationLayerRules,
  getLocationType,
} from "./LocationType";
...
export class AssetData implements AssetLocationEntity {
  ...
  // 從 MongoDB 模型轉換
  static fromMongoModel(model: MongoLocationDataModel): AssetData {
    const assetType = getLocationType(model.type);
  ...
  // 轉換為 MongoDB 模型
  toMongoModel(): MongoLocationDataModel {
    return {
  ...
  static createNew(type: LocationType, ancestors: string[] = []): AssetData {
    return new AssetData({
      id: undefined,
      name: `New ${type}`,
      description: "",
